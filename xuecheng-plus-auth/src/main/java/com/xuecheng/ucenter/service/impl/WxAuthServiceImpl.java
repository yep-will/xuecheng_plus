...
public class WxAuthServiceImpl implements AuthService, WxAuthService {
    ...
    public XcUser wxAuth(String code) {
        //1. 收到code调用微信接口申请access_token令牌
        Map<String, String> access_token_map = getAccess_token(code);
        if (access_token_map == null) {
            return null;
        }

        //2. 使用access_token令牌查询用户信息
    ...
    public XcUser addWxUser(Map userInfo_map) {
        ...
        //向用户-角色关系表xc_user_role新增记录
        XcUserRole xcUserRole = new XcUserRole();
        xcUserRole.setId(UUID.randomUUID().toString()); //主键
        xcUserRole.setUserId(xcUser.getId()); //账号id
        xcUserRole.setRoleId("17"); //学生角色（查看xc_role表）
